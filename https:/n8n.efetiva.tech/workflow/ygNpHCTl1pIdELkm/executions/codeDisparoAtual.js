// ====================================================================================
// CÓDIGO PARA O NÓ "CODE" DO N8N - GERADOR DE INTERFACE (v17 - Nova Estrutura de Dados)
// Autor: Gemini
// Descrição: Adapta a ferramenta para a nova estrutura de dados de entrada,
//            com novas colunas, busca multi-campo e lógica de fallback para nomes.
// ====================================================================================

// Pega os dados do nó anterior e mapeia para a variável 'contacts'.
// Preparado para o modo "Run Once for All Items".
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // ===================================================================
            const formatPhone = (fone_partic_raw) => fone_partic_raw ? fone_partic_raw.split('@')[0] : '';
            const toTitleCase = (str) => str ? str.trim().split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ') : '';
            ...
            (() => {
                ...
                // *** ALTERAÇÃO 1: NOVA ESTRUTURA DO CABEÇALHO DA TABELA ***
                wrapper.innerHTML = \`<div class="contacts-table-container">
                    <h2 style="border-radius: 6px 6px 0 0;">CONTATOS DISPONÍVEIS</h2>
                    <div style="padding: 1rem; background-color: var(--card-bg-color); border-radius: 0 0 6px 6px;">
                        <input type="text" id="searchInput" placeholder="Pesquisar por nome, telefone ou grupo...">
                        <div class="table-scroll-wrapper" style="border: none; border-radius: 0;">
                            <table id="contactsTable">
                                <thead>
                                    <tr>
                                        <th><input type="checkbox" id="selectAllCheckbox" title="Selecionar todos os visíveis"></th>
                                        <th>AÇÕES</th>
                                        <th data-column="nome_partic">NOME <span class="sort-indicator"></span></th>
                                        <th data-column="fone_partic">TELEFONE <span class="sort-indicator"></span></th>
                                        <th data-column="qtd_grps">QTD. GRUPOS <span class="sort-indicator"></span></th>
                                        <th data-column="info_grp_wa">INFO GRUPO <span class="sort-indicator"></span></th>
                                    </tr>
                                </thead>
                                <tbody></tbody>
                            </table>
                        ...
                        <div id="pagination-controls">
                            ...
                            <button id="nextPageBtn" class="btn btn-secondary">Próxima &gt;</button>
                        </div>
                    </div>
                </div>
                <div class="selected-contacts-container">
                    <h2>SELECIONADOS (<span id="selectedCount">0</span>/<span id="maxCount">${maxSelectionsFinal}</span>)</h2>
                    <div class="table-scroll-wrapper" style="border-radius: 6px;">
                        <table id="selectedTable">
                            <thead><tr><th>#</th><th>AÇÃO</th><th>NOME</th></tr></thead>
                            <tbody></tbody>
                        </table>
                    </div>
                </div>\`;
                ...
                let sortState = { column: 'nome_partic', direction: 'asc' };
                let currentPage = 1;
                ...
                function renderPage() {
                    ...
                    pageData.forEach(c => {
                        ...
                        // *** ALTERAÇÃO 2: LÓGICA DE NOME E TELEFONE ***
                        const phone = formatPhone(c.fone_partic);
                        const displayName = toTitleCase(c.nome_partic) || phone; // Usa o telefone se o nome for nulo/vazio
                        
                        tr.innerHTML = \`<td><input type="checkbox" class="contact-checkbox" \${isSelected ? 'checked' : ''} \${!isSelected && limit ? 'disabled' : ''}></td>
                                        <td>\${createWhatsAppButtonHTML(phone)}</td>
                                        <td>\${displayName}</td>
                                        <td>\${phone}</td>
                                        <td>\${c.qtd_grps}</td>
                                        <td>\${c.info_grp_wa}</td>\`;
                ...
                function renderSelectedTable() { 
                    ...
                    let counter = 1; 
                    contacts.forEach(c => { 
                        if (selectedContactIds.has(c.id.toString())) { 
                            const tr = document.createElement('tr'); 
                            
                            // *** ALTERAÇÃO 3: LÓGICA DE NOME E TELEFONE NA TABELA DE SELECIONADOS ***
                            const phone = formatPhone(c.fone_partic);
                            const displayName = toTitleCase(c.nome_partic) || phone;
                            
                            tr.innerHTML = \`<td>\${counter++}.</td><td>\${createWhatsAppButtonHTML(phone)}</td><td>\${displayName}</td>\`; 
                            selectedTableBody.appendChild(tr); 
                        } 
                    }); 
                }

                function applySort() { const { column, direction } = sortState; filteredData.sort((a, b) => { const vA = a[column], vB = b[column]; if (vA === null || vA === undefined) return 1; if (vB === null || vB === undefined) return -1; let c = 0; if (typeof vA === 'string') c = vA.localeCompare(vB); else if (vA > vB) c = 1; else if (vA < vB) c = -1; return direction === 'asc' ? c : c * -1; }); }
                ...
                // *** ALTERAÇÃO 4: LÓGICA DE BUSCA MULTI-CAMPO ***
                searchInput.addEventListener('input', (e) => { 
                    const searchTerm = e.target.value.toLowerCase();
                    filteredData = contacts.filter(c => {
                        const name = c.nome_partic || '';
                        const phone = c.fone_partic || '';
                        const groupInfo = c.info_grp_wa || '';
                        return name.toLowerCase().includes(searchTerm) || 
                               phone.toLowerCase().includes(searchTerm) || 
                               groupInfo.toLowerCase().includes(searchTerm);
                    });
                    applySort(); 
                    currentPage = 1; 
                    renderPage(); 
            ...
            (() => {
                ...
                function generateSummary() {
                    ...
                    selectedContactIds.forEach(id => {
                        ...
                        // *** ALTERAÇÃO 5: LÓGICA DE NOME NO RESUMO FINAL ***
                        const displayName = toTitleCase(contact.nome_partic) || formatPhone(contact.fone_partic);
                        const nome = displayName.split(' ')[0]; // Pega só o primeiro nome para a saudação
                        ...
                        summaryData.push({ 
                            id: contact.id, 
                            nome: displayName, // Salva o nome completo ou telefone
                            telefone: formatPhone(contact.fone_partic), 
                            mensagem: [saudacaoFinal, corpoFinal, ctaFinal].filter(Boolean).join('\\n') 
                        });
                    });
                ...
                function renderSummaryTable() {
                    ...
                    summaryData.forEach((item, index) => {
                        ...
                        row.innerHTML = \`<div class="contact-info"><span class="count">\${index + 1}.</span>\${createWhatsAppButtonHTML(item.telefone)}<span>\${item.nome}</span></div><div class="contact-message"><input type="checkbox" class="edit-message-checkbox" title="Editar esta mensagem"><div class="message-content">\${item.mensagem.replace(/\\n/g, '<br>')}</div></div>\`;
                ...
                confirmDispatchBtn.addEventListener('click', async () => {
                    ...
                    try {
                        const response = await fetch('https://webhook.efetiva.tech/webhook/executa-disparo-efetiva', {
